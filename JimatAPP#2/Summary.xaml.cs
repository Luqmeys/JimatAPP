...
namespace JimatAPP_2
{
    ...
    {
        public List<(string Label, decimal Value, Color Slice)> Segments { get; set; } = new();
    ...
    public partial class Summary : ContentPage
    {
        private static readonly Color[] Palette =
        {
            ...
            Color.FromArgb("#FB923C"),
            Color.FromArgb("#38BDF8"),
            Color.FromArgb("#E879F9"),
        ...
        private static readonly Color MutedText = Color.FromArgb("#888888");
        ...
        {
            InitializeComponent();
            PieChart.Drawable = _pieDrawable;
        }
        ...
        private void OnIncomeTabTapped(object sender, EventArgs e)
        {
            if (_tab == Tab.Income) return;
            _tab = Tab.Income;
            UpdateTabUI();
            Refresh();
        }
        ...
        {
            bool isExp = _tab == Tab.Expenses;
            ExpensesTabLabel.TextColor = isExp ? Colors.White : MutedText;
            ExpensesTabUnderline.BackgroundColor = isExp ? Colors.White : Colors.Transparent;
            IncomeTabLabel.TextColor = !isExp ? Colors.White : MutedText;
            IncomeTabUnderline.BackgroundColor = !isExp ? Colors.White : Colors.Transparent;
            TotalCardTitle.Text = isExp ? "Total Expenses" : "Total Income";
        }
        ...
        private void OnAllTapped(object sender, EventArgs e)
        {
            ...
            _period = Period.All; _periodOffset = 0; UpdatePillUI(); Refresh();
        }

        private void UpdatePillUI()
        {
            WeekPillLabel.TextColor = MutedText;
            MonthPillLabel.TextColor = MutedText;
            YearPillLabel.TextColor = MutedText;
            AllPillLabel.TextColor = MutedText;

            switch (_period)
            {
                case Period.Week: WeekPillLabel.TextColor = GreenAccent; break;
                case Period.Month: MonthPillLabel.TextColor = GreenAccent; break;
                case Period.Year: YearPillLabel.TextColor = GreenAccent; break;
                default: AllPillLabel.TextColor = GreenAccent; break;
            }

            Grid.SetColumn(ActivePill, _period switch
            {
                Period.Week => 0,
                Period.Month => 1,
                Period.Year => 2,
        ...
        private void OnNextPeriodTapped(object sender, EventArgs e)
        {
            if (_period == Period.All || _periodOffset >= 0) return;
            _periodOffset++;
            Refresh();
        }
        ...
        private (DateTime start, DateTime end, string label) GetDateRange()
        {
            ...
            {
                var ws = today.AddDays(-(int)today.DayOfWeek).AddDays(_periodOffset * 7);
                var we = ws.AddDays(6);
                string lbl = _periodOffset == 0 ? $"This week · {ws:MMM d} – {we:MMM d}"
                           : _periodOffset == -1 ? $"Last week · {ws:MMM d} – {we:MMM d}"
                           : $"{ws:MMM d} – {we:MMM d, yyyy}";
                return (ws, we, lbl);
            }

            if (_period == Period.Month)
            {
                var ref2 = today.AddMonths(_periodOffset);
                var ms = new DateTime(ref2.Year, ref2.Month, 1);
                var me = ms.AddMonths(1).AddDays(-1);
                string lbl = _periodOffset == 0 ? $"This month · {ms:MMMM yyyy}"
                           : _periodOffset == -1 ? $"Last month · {ms:MMMM yyyy}"
                           : ms.ToString("MMMM yyyy");
                return (ms, me, lbl);
            }

            int year = today.Year + _periodOffset;
            return (new DateTime(year, 1, 1), new DateTime(year, 12, 31),
                    _periodOffset == 0 ? $"This year · {year}" : year.ToString());
        }
            ...
            where T : class
        {
            ...
            DrawPieAndLegend(groups, total);
            DrawBreakdown(groups, total);
        }
            ...
            List<(string Category, decimal Amount)> groups, decimal total)
        {
            ...
            _pieDrawable.Segments.Clear();

            bool hasData = groups.Count > 0 && total > 0;
            PieChart.IsVisible = hasData;
            EmptyChartLabel.IsVisible = !hasData;
            ...
            for (int i = 0; i < groups.Count; i++)
            {
                ...
                _pieDrawable.Segments.Add((g.Category, g.Amount, color));

                double pct = (double)(g.Amount / total * 100);

                var row = new Grid();
                row.ColumnDefinitions.Add(new ColumnDefinition { Width = 14 });
            ...
            List<(string Category, decimal Amount)> groups, decimal total)
        {
            ...
            for (int i = 0; i < groups.Count; i++)
            {
                ...
                barGrid.HeightRequest = 6;

                var barFill = new BoxView { BackgroundColor = color, HeightRequest = 6, CornerRadius = 4 };
                var barEmpty = new BoxView { BackgroundColor = BarBg, HeightRequest = 6, CornerRadius = 4 };
                ...
                var pctLbl = new Label
                {
                    Text = $"{pct:F1}% of total",
                    FontSize = 11,
                    TextColor = MutedText
                };

                layout.Children.Add(topRow);
                layout.Children.Add(barGrid);
                layout.Children.Add(pctLbl);
                card.Content = layout;
                BreakdownStack.Children.Add(card);
            }
        }
